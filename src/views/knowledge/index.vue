  ...
  <el-container>
    ...
    <el-container>
      <el-header style="height: 110px;">
        ...
        <el-row class="todo-title">
          <el-col :span="1">
            <i class="el-icon-back char-margin" @click="goBack" />
          </el-col>
          <el-col :span="1">
            <i class="el-icon-right char-margin" @click="goFoward" />
          </el-col>
          <el-col :span="1">
          ...
          <el-col :span="3">
            <el-dropdown trigger="click">
              ...
              <el-dropdown-menu slot="dropdown">
                <div @click="createFolder()"><el-dropdown-item><i class="el-icon-folder" />新建文件夹</el-dropdown-item></div>
                <div @click="createDoc()"><el-dropdown-item><i class="el-icon-document" />新建文档</el-dropdown-item></div>
              </el-dropdown-menu>
            </el-dropdown>
          </el-col>
        </el-row>
        ...
        <el-table v-loading="loading" :data="tableData">
          <el-table-column label="名称">
            <template slot-scope="scope">
              <div v-if="scope.row.is_dir==1" style="cursor: pointer" @click="getList(scope.row.cate_id,scope.row.id)">
          ...
          <el-table-column label="作者">
            <template slot-scope="scope">
              <span v-show="!scope.row.is_dir" style="margin-left: 10px">{{ scope.row.author }}</span>
            </template>
          </el-table-column>
          <el-table-column label="复核人">
            <template slot-scope="scope">
              <span v-show="!scope.row.is_dir" style="margin-left: 10px">{{ scope.row.review_user }}</span>
            </template>
          </el-table-column>
          <el-table-column label="审核状态" width="120">
            <template v-if="!scope.row.is_dir" slot-scope="scope">
              <i v-if="scope.row.status=='3'" class="el-icon-success" />
              <i v-else class="el-icon-time" @click="check(scope.row.id)" />
            </template>
          </el-table-column>
        </el-table>
        <!-- 分页 -->
    ...
    <el-dialog title="新建文件夹" :visible.sync="createDialogVisible" width="20%" :close-on-click-modal="false" @close="cancel();return true;">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" size="" @click="saveFile()">确 定</el-button>
...
export default {
  ...
  methods: {
    ...
    dirSearch(filename) {
      // 向后台发送请求获取数据；
      const params = { keywords: filename }
      this.curd.getHotTitles(params)
        .then(response => {
          // 成功执行内容
          this.tableData = response.data
